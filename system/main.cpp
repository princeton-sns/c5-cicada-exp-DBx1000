...
int main(int argc, char* argv[]) {
            ...
            threads.emplace_back([&, thread_id] {
                ...
                for (auto it : m_wl->tables) {
                    ...
                    for (auto mica_tbl : table->mica_tbl) {
                        ...
                        uint64_t i = 0;
                        mica_tbl->renew_rows(m_wl->mica_db->context(thread_id),
                                             0, i, static_cast<uint64_t>(-1),
                                             false);
                ...
                for (auto it : m_wl->hash_indexes) {
                    ...
                    for (auto idx : index->mica_idx) {
                        ...
                        if (thread_id == 0) {
                            printf("thread %2" PRIu64
                                   ": index %s part %2" PRIu64 ":\n",
                                   thread_id, it.first.c_str(), part_id - 1);
                            mica_tbl->print_table_status();
                        }
                    }
    ...
    // int ret = system("perf record -a -o perf.data -g -e LLC-load-misses sleep
    // 1 &"); int ret = system("perf record -C 0 -o perf.data sleep 1 &"); int
    // ret = system("perf record -C 1 -o perf.data sleep 1 &"); int ret =
    // system("perf stat -a -e
    // cycles,instructions,cache-references,cache-misses,branches,branch-misses,stalled-cycles-frontend,L1-dcache-load-misses,L1-dcache-store-misses,L1-icache-load-misses,LLC-loads,LLC-load-misses,LLC-stores,LLC-store-misses,dTLB-load-misses,dTLB-store-misses,iTLB-loads,iTLB-load-misses,node-stores,node-load-misses,node-stores,node-store-misses
    // sleep 1 &"); int ret = system("perf stat -C 1 sleep 1 &"); int ret =
    // system("perf stat -C 2 sleep 1 &"); (void)ret;
...
#if MICA_CCC != MICA_CCC_NONE
    {
        ...
        std::vector<std::size_t> nentries;
        for (auto pair : logdirs) {
            auto relaydir = pair.second;
            printf("Preprocessing logs in %s\n", relaydir.c_str());
            auto n = ccc->preprocess_logs(pair.first, relaydir);
            nentries.emplace_back(n);
        }

        for (std::size_t i = 0; i < logdirs.size(); ++i) {
            switch (i) {
                case 0:
            ...
            for (const auto& item : mica_replica->get_tables_index()) {
                printf("table %s :\n", item.first.c_str());
                mica_replica->get_table_by_index(item.second)
                    ->print_table_status();
            }
            ...
            for (uint32_t i = 0; i < g_worker_cnt; i++)
                inter_commit_latency +=
                    mica_replica->context(static_cast<uint16_t>(i))
                        ->inter_commit_latency();
